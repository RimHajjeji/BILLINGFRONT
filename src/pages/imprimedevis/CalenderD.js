...
import { HomeD, Upd_etat_devis } from '../../store/homeD/action';
...
function CalenderD() {
    ...
    const [deviss, setDeviss] = React.useState('')
    ...
    const update = () => {
        dispatch(Upd_etat_devis(selectedDevis?._id, !selectedDevis?.etats, Idcopany))
        handerClose()
    }
    const delate = () => {
        dispatch(del_Devis(deletes?._id, Idcopany))
        handerClose1()
    }
    const keys = [
        "raison_sociale",
    ...
    useEffect(() => {
        if (deviss?.length > 0) {
            setFiltredDevis(devis?.filter((el) => {
                return (
                    keys.some((key) => el?.clientId?.[key]?.toLowerCase()?.includes(String(deviss)) === true)
    ...
    return (
        ...
        <div style={{ marginTop: '60px' }}>
            <div className="d-grid gap-2">
                ...
                <Form.Group value={deviss} onChange={(e) => setDeviss(e.target.value)} style={{ marginTop: '40px', marginBottom: '40px' }} controlId="text">
